...
class FeatureConfig:
    ...
    # Horizon resampling — Step 120: TESTING 15m-only consensus as clean baseline.
    # No horizon features — pure 15m price/volume/technical data.
    # This establishes whether multi-horizon features add value or hurt performance.
    HORIZONS = {
        "15m": 1,  # base horizon only
    }
...
class ModelConfig:
    ...
    RANDOM_FOREST_N_ESTIMATORS = [
        200,
        300,
    ]  # increased from [100,200] (Step 47): larger ensembles → more stable predictions
    RANDOM_FOREST_MAX_DEPTH = [
        12,
        15,
        18,
    ]  # increased from [10,12,15] (Step 49): deeper trees with 200-300 estimators
    RANDOM_FOREST_MIN_SAMPLES_LEAF = [
        5,
        10,
        15,
    ]  # expanded from [10, 15]: allow finer splits
    ...
    EXTRA_TREES_N_ESTIMATORS = [
        200,
        300,
    ]  # increased from [100,200] (Step 47): larger ET ensembles for more stability
    EXTRA_TREES_MAX_DEPTH = [
        12,
        15,
        18,
    ]  # increased from [10,12,15] (Step 49): deeper ET trees
    ...
    GRADIENT_BOOSTING_LEARNING_RATE = [
        ...
        0.05,
        0.1,
    ]  # added 0.02 (slower, more careful learning)
    GRADIENT_BOOSTING_N_ESTIMATORS = [
        200,
        300,
        500,
    ]  # increased from [100,200,300] (Step 48): larger HistGBM ensembles
    ...
    LIGHTGBM_N_ESTIMATORS = [
        ...
        200,
        300,
    ]  # reduced: 400 was overfitting on leaky data
    LIGHTGBM_LEARNING_RATE = [0.02, 0.05, 0.1]  # full range maintained
    LIGHTGBM_NUM_LEAVES = [
        15,
        31,
        63,
    ]  # reduced: 127 was overfitting; smaller trees generalize better
    LIGHTGBM_MIN_CHILD_SAMPLES = [
        20,
        50,
        100,
    ]  # increased: more min_child helps generalization
    LIGHTGBM_REG_ALPHA = [
        0.0,
        0.1,
        0.5,
        1.0,
    ]  # Step 85: L1 regularization to prevent overfitting
    LIGHTGBM_REG_LAMBDA = [
        0.0,
        0.1,
        0.5,
        1.0,
    ]  # Step 85: L2 regularization to prevent overfitting
    ...
    CALIBRATION_METHOD = (
        "sigmoid"  # 'sigmoid' (Platt) = stable; 'isotonic' = better on large datasets
    )
...
class TradingConfig:
    ...
    FEE_RATE = 0.0002  # 0.02% per side (maker fee — mean-reversion limit orders)
    # With 0% slippage + 0.04% round-trip: break-even at 1h scale = 53.6% (much lower than 61.8% at 15m)
    # Avg 1h move ≈ 0.55% → break-even = 0.5 + 0.02/0.55 = 53.6%
    SLIPPAGE = (
        0.0  # 0% slippage: realistic for BTC perp limit orders at our trade size ($500)
    )
    ...
    MAX_POSITIONS = 1
    STOP_LOSS_PCT = 0.0  # No stop-loss for now
    HOLD_PERIODS = (
        2  # Step 112: 2-bar hold (30m) — lower break-even (54.2% vs 61.8% at 1-bar)
    )
